...
class Transformer(object):
    ...
    def __init__(self, k8s_client_dict):
    ...
    def create_k8s_object(self, kind, file_content_dict):
        must_param = {
        ...
        self.init_k8s_obj(k8s_obj, file_content_dict, must_param)
        return k8s_obj
        ...
        for artifact_file in artifact_files:
            ...
            if ((urlparse(artifact_file).scheme == 'file') or
                ...
                with open(artifact_file_path, 'r') as f:
                    file_content = f.read()
            file_content_dicts = yaml.safe_load_all(file_content)
            for file_content_dict in file_content_dicts:
                kind = file_content_dict.get('kind')
                k8s_obj['object'] = self.create_k8s_object(
                    kind, file_content_dict)
                if not file_content_dict.get('metadata'):
                    k8s_obj['namespace'] = ''
                elif file_content_dict.get('metadata').get('namespace'):
                    k8s_obj['namespace'] = \
    ...
    def select_k8s_client_and_api(
            self, kind, namespace, apiVersion, body):
        value = {
            "Pod":
                'self.k8s_client_dict[apiVersion].create_namespaced_pod',
            "Service":
                'self.k8s_client_dict[apiVersion].'
                'create_namespaced_service',
            "ConfigMap":
                'self.k8s_client_dict[apiVersion].'
            'create_namespaced_config_map',
            "Secret":
                'self.k8s_client_dict[apiVersion].create_namespaced_secret',
            "PersistentVolumeClaim ":
                'self.k8s_client_dict[apiVersion].'
            ...
            "Namespace":
                'self.k8s_client_dict[apiVersion].create_namespace',
            "Node":
                'self.k8s_client_dict[apiVersion].create_node',
            "PersistentVolume":
                'self.k8s_client_dict[apiVersion].'
            ...
            "APIService":
                'self.k8s_client_dict[apiVersion].create_api_service',
            "DaemonSet":
                'self.k8s_client_dict[apiVersion].'
            ...
            "TokenReview":
                'self.k8s_client_dict[apiVersion].create_token_review',
            ...
            "Job":
                'self.k8s_client_dict[apiVersion].create_namespaced_job',
            "Lease":
                'self.k8s_client_dict[apiVersion].create_namespaced_lease',
            "NetworkPolicy":
                'self.k8s_client_dict[apiVersion].'
            'create_namespaced_network_policy',
            "ClusterRole":
                'self.k8s_client_dict[apiVersion].create_cluster_role',
            "ClusterRoleBinding":
                'self.k8s_client_dict[apiVersion].'
            ...
            "PriorityClass":
                'self.k8s_client_dict[apiVersion].create_priority_class',
            "StorageClass":
                'self.k8s_client_dict[apiVersion].create_storage_class',
            "VolumeAttachment":
                'self.k8s_client_dict[apiVersion].'
                'create_volume_attachment',
        }
        response = eval(value.get(kind))(namespace=namespace, body=body)
        return response
    ...
    def deploy(self, kubernetes_objects):
        ...
        """
        kubernetes_objects = self.sort_k8s_obj(kubernetes_objects)
        new_k8s_objs = list()
        for kubernetes_object in kubernetes_objects:
            ...
            apiVersion = kubernetes_object.get('object').api_version
            body = kubernetes_object.get('object')
            name = kubernetes_object.get('object').metadata.name
            try:
                self.select_k8s_client_and_api(
                    kind, namespace, apiVersion, body)
                kubernetes_object['status'] = 'Creating'
            except Exception as e:
                kubernetes_object['status'] = 'creating_failed'
                LOG.error('''The request to create a resource failed.
    ...
    def get_lower_case_name(self, name):
        name = name.strip()
        return re.sub('([a-z0-9])([A-Z])', r'\1_\2', name).lower()

    def init_k8s_obj(self, obj, content, must_param):
        for key, value in content.items():
            param_value = self.get_lower_case_name(key)
            if hasattr(obj, param_value) and \
                ...
                if obj_name == 'dict(str, str)':
                    ...
                    if obj_name in must_param.keys():
                        ...
                        rely_obj = eval('client.' + obj_name + '()')
                    self.init_k8s_obj(rely_obj, value, must_param)
                ...
                if obj_name == 'list[str]':
                    ...
                    for v in value:
                        ...
                        self.init_k8s_obj(rely_obj, v, must_param)
                        rely_objs.append(rely_obj)
                    setattr(obj, param_value, rely_objs)

    def sort_k8s_obj(self, k8s_objs):
        flag = 0
        objs = k8s_objs
        new_k8s_objs = list()
        for inx, i in enumerate(self.SORT):
            for obj_inx, obj in enumerate(objs):
                if obj["object"].kind == i:
                    new_k8s_objs.append(objs.pop(obj_inx))
            if inx == 8:
                flag = len(new_k8s_objs)
        for obj in objs:
            new_k8s_objs.insert(flag, obj)
