...
class TestTransformer(base.TestCase):
    ...
    def test_deploy_create_false(self):
        kubernetes_objects = []
        k8s_obj = fakes.fake_k8s_dict()
        kubernetes_objects.append(k8s_obj)
        k8s_objs = self.transfromer.deploy(kubernetes_objects)
        self.assertEqual(k8s_objs[0]['status'], 'creating_failed')

    @mock.patch.object(translate_outputs.Transformer,
                       "select_k8s_client_and_api")
    def test_deploy(self, mock_k8s_client_and_api):
        req = \
        ...
        k8s_obj = fakes.fake_k8s_dict()
        kubernetes_objects.append(k8s_obj)
        new_k8s_objs = self.transfromer.deploy(kubernetes_objects)
